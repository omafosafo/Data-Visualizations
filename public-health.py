...
with tabs[2]:
    ...
    #health_events = st.sidebar.multiselect('Select Health Event(s)', sorted([event for event in df_selected['Health Event Under Surveillance'].dropna().unique() if event.lower() != 'others']))
    ...
    if filtered_df is not None and not filtered_df.empty:
        ...
        with view2:
            ...
            if health_event_counts.empty:
                st.warning("No data available to show for the selected filters.. Please adjust your filters.")
            else:
        ...
        with view3:
            if 'Platform #1' in filtered_df.columns and not filtered_df['Platform #1'].dropna().empty:
                platform_counts = filtered_df['Platform #1'].value_counts()
                if not platform_counts.empty:
                    most_used_platform = platform_counts.idxmax()
                    most_used_platform_count = platform_counts.max()
                else:
                    most_used_platform = "N/A"
                    most_used_platform_count = 0
            else:
                most_used_platform = "N/A"
                most_used_platform_count = 0
           #if 'Platform #1' in filtered_df.columns and not filtered_df['Platform #1'].dropna().empty:
            ...
            #    most_used_platform = "N/A"
             #   st.warning("No data available to show for the selected filters. Please adjust your filters.")
        with view4:
            if most_used_platform_count.empty:
                st.warning("No data available to show for the selected filters. Please adjust your filters.")
            else:
        ...
        with col1:
            if country_counts.empty:
                st.warning("No data available to show for the selected filters. Please adjust the filters.")
            else:
